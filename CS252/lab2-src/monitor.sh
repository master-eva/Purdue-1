...
function notify
{
	...
	if [ $MEM_THRESHOLD -eq -1 ] 
	then
		if [ $cpu_usage_int -gt $CPU_THRESHOLD ]
		then
			`/usr/bin/mailx -s "CPU exceeds limit" $USER < ./reports_dir/$file_name`
		fi
	elif [ $cpu_usage_int -gt $CPU_THRESHOLD ] && [ $mem_usage_int -gt $MEM_THRESHOLD ]
	then 
		`/usr/bin/mailx -s "CPU and MEM exceeds limit" $USER < ./reports_dir/$file_name`
	elif [ $cpu_usage_int -gt $CPU_THRESHOLD ]
	then
		`/usr/bin/mailx -s "CPU exceeds limit" $USER < ./reports_dir/$file_name`
	elif [ $mem_usage_int -gt $MEM_THRESHOLD ]
	then
		`/usr/bin/mailx -s "MEM exceeds limit" $USER < ./reports_dir/$file_name` 
	fi
