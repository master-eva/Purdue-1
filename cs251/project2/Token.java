...
public class Token {
    ...
    public boolean equals(Object token) {
		try {
            Token t = (Token) token;
            if (this.getToken() != t.getToken()) return false;
            if (this.getType() != t.getType()) return false;

            switch (this.getType()) {
                case 'i':
                    return (this.getValue_i() == t.getValue_i());
                case 'f':
                    return (this.getValue_f() - t.getValue_f() < 0.000000001 &&
                            this.getValue_f() - t.getValue_f() > -0.000000001);
                case 's':
                    return (this.getValue_s().equals(t.getValue_s()));
                default:
                    return false;
            }
        } catch (Exception e) {
